...
def run_catering_analysis():
    ...
    with pd.ExcelWriter(report_file, engine="openpyxl") as writer:
        ...
        # 7_优惠分析（总体占比 / 类型金额占比 / 原因分布 / 分布维度 / 用户/菜品/员工）
        sheet = "7_优惠分析"
        pd.DataFrame([["核心结论"]]).to_excel(writer, sheet_name=sheet, startrow=0, index=False, header=False)
        r = 2
        # 总体优惠占比
        df_disc_orders = (
            df_discounts.groupby("id")["discount_amount"].sum().reset_index().rename(
                columns={"discount_amount": "优惠金额"}
            )
        )
        ...
        overall_disc.to_excel(writer, sheet_name=sheet, startrow=r + 1, index=False)
        ...
        # 将订单级优惠额与支付时间合并，用于日期/星期/时段分布 & 用户特征 & 菜品/员工分析
        df_disc_orders_time = pd.merge(
            df_disc_orders,
            df_pay[["id", "time", "payer"]],
            on="id",
            how="left",
        )
        if not df_disc_orders_time.empty:
            df_disc_orders_time["time"] = pd.to_datetime(df_disc_orders_time["time"], errors="coerce")
            df_disc_orders_time["date"] = df_disc_orders_time["time"].dt.normalize()
            # 优惠日期分布
            disc_by_date = (
                df_disc_orders_time.groupby("date")
                .agg({"优惠金额": "sum", "id": "nunique"})
                .reset_index()
                .rename(columns={"date": "日期", "id": "订单数"})
            )
            # 优惠星期分布
            df_disc_orders_time["weekday"] = df_disc_orders_time["time"].dt.weekday
            ...
            disc_by_weekday = (
                ...
                .agg({"优惠金额": "sum", "id": "nunique"})
                .reset_index()
                .rename(columns={"weekday_cn": "星期", "id": "订单数"})
            )
            # 优惠时段分布
            df_disc_orders_time["daypart"] = df_disc_orders_time["time"].apply(classify_daypart)
            disc_by_daypart = (
                df_disc_orders_time.groupby("daypart")
                .agg({"优惠金额": "sum", "id": "nunique"})
                .reset_index()
                .rename(columns={"daypart": "时段", "id": "订单数"})
            )
        else:
            disc_by_date = pd.DataFrame(columns=["日期", "优惠金额", "订单数"])
            disc_by_weekday = pd.DataFrame(columns=["星期", "优惠金额", "订单数"])
            disc_by_daypart = pd.DataFrame(columns=["时段", "优惠金额", "订单数"])

        # 优惠用户特征(谁/几次/金额) — 保持在 with 内以便后续写入
        if not df_disc_orders_time.empty:
            user_disc = (
            ...
            user_disc = user_disc.merge(user_rfm[["payer", "segment"]], on="payer", how="left")
        ...
        # 优惠关联菜品 Top15：订单级优惠额分摊到涉及的菜品（统计维度：菜品）
        if not df_disc_orders.empty:
            df_disc_dish = pd.merge(
                df_sales[["id", "dish"]],
                df_disc_orders,
                on="id",
                how="inner",
            ...
            disc_dish = (
                ...
                .agg({"优惠金额": "sum", "id": "nunique"})
                .reset_index()
                .rename(columns={"dish": "菜品", "id": "订单数"})
            )
            disc_dish_top15 = disc_dish.sort_values("优惠金额", ascending=False).head(15)
        else:
        ...
        if not df_disc_orders.empty:
            exclude_pattern = r"00000|00015|顾客/系统|收银"
            staff_per_order = df_wide[["id", "staff"]].copy()
            staff_per_order = staff_per_order[
                ~staff_per_order["staff"].astype(str).str.contains(exclude_pattern, na=False)
            ]
            staff_per_order = staff_per_order.drop_duplicates(subset=["id"])
            df_disc_staff = pd.merge(df_disc_orders, staff_per_order, on="id", how="left").dropna(subset=["staff"])
            staff_disc = (
            ...
            staff_orders = staff_report[["staff", "订单数"]].rename(columns={"订单数": "总订单数"})
            staff_disc = staff_disc.merge(staff_orders, on="staff", how="left")
            staff_disc["优惠订单占比"] = staff_disc["优惠订单数"] / staff_disc["总订单数"]
        ...
        # 写入 6 个新增表（与 df_disc_orders 是否为空无关，始终执行）
        r = r + len(type_reason_dist) + 3
        pd.DataFrame([["【优惠日期分布】"]]).to_excel(
            writer, sheet_name=sheet, startrow=r, index=False, header=False
        )
        disc_by_date.to_excel(writer, sheet_name=sheet, startrow=r + 1, index=False)
        r = r + len(disc_by_date) + 3
        pd.DataFrame([["【优惠星期分布】"]]).to_excel(
            writer, sheet_name=sheet, startrow=r, index=False, header=False
        )
        disc_by_weekday.to_excel(writer, sheet_name=sheet, startrow=r + 1, index=False)
        r = r + len(disc_by_weekday) + 3
        pd.DataFrame([["【优惠时段分布】"]]).to_excel(
            writer, sheet_name=sheet, startrow=r, index=False, header=False
        )
        disc_by_daypart.to_excel(writer, sheet_name=sheet, startrow=r + 1, index=False)
        r = r + len(disc_by_daypart) + 3

        pd.DataFrame([["【优惠用户特征(谁/几次/金额)】"]]).to_excel(
            writer, sheet_name=sheet, startrow=r, index=False, header=False
        )
        disc_user_feature.to_excel(writer, sheet_name=sheet, startrow=r + 1, index=False)
        ...
        disc_staff_summary.to_excel(writer, sheet_name=sheet, startrow=r + 1, index=False)
